...
class PlanetInfoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        GlobalScope.launch(Dispatchers.Main) {
            ...
            if (sunriseTime != null && sunsetTime != null) {
                ...
                val sunriseLabel = getLocalizedString(applicationContext, R.string.SunriseTime)
                val localizedSunrise = getLocalizedTime(sunriseTime, this@PlanetInfoActivity)
                val sunsetLabel = getLocalizedString(applicationContext, R.string.SunsetTime)
                val localizedSunset = getLocalizedTime(sunsetTime, this@PlanetInfoActivity)

                // Display localized times on TextViews
                binding.textViewSunrise.text = "$sunriseLabel $localizedSunrise"
                binding.textViewSunset.text = "$sunsetLabel $localizedSunset"
            }
        }
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId){
            R.id.action_english -> {
                setLocale(applicationContext, "en")
                recreate()
            }
            R.id.action_chinese -> {
                setLocale(applicationContext, "zh")
                recreate()
            }
        }
